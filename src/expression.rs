use crate::{ByteString, Counters, CounterContext, CounterContextMut, LocalCounter};
        ...
        match self {
            ...
            ExprState::Operator(op) => {
                ...
                if byte == b' ' {
                    ...
                    Ok((ExprState::Constant((byte - b'0') as i32), Some(Token::Operator(op))))
                } else if byte == b'\'' {
            ...
            ExprState::VariableName(var) => {
                ...
                if byte == b' ' {
                    ...
                    Ok((ExprState::Operator(ExprOp::Plus), Some(Token::Variable(var))))
                } else if byte == b'-' {
                    Ok((ExprState::Operator(ExprOp::Minus), Some(Token::Variable(var))))
                } else {
                    Err(())
                }
            }
...
pub(crate) fn evaluate_expression(
    expr: &[u8],
    _counters: &Counters,
    _context: &dyn CounterContextExt
) -> ByteString {
...
mod test {
    ...
    fn expr_var() {
        ...
        counters.set("countername".into(), &TestLocalCounters, 5);
        assert_eq!(evaluate_expression(b"'countername'", &Counters::new(), &TestLocalCounters).0, b"-5");
    }
}
